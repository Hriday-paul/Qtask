...
import { divisions } from '@/utils/config'
        ...
        <form onSubmit={handleSubmit} className='flex flex-row gap-x-1 items-center justify-between py-2 md:py-0'>
            <div className='flex flex-row gap-x-1 items-center'>
                ...
                <input
                    ...
                    type="text" name='search' className='border-none outline-0 focus:outline-0 text-black font-epilogue' placeholder="search..." />
            </div>

            <button type='submit' className='bg-primary text-white px-3 py-2 rounded font-epilogue cursor-pointer hover:opacity-70 duration-200'>Search</button>
        ...
        <Dialog>
            <DialogTrigger asChild>
                <button className={` py-3.5 md:py-0 flex flex-row gap-x-1 items-center border-b md:border-b-0 md:border-r border-stroke cursor-pointer ml-2 w-full ${district || division || area ? "text-black" : "text-gray-400"}`}>
                    ...
                    <p className='text-base font-epilogue'>{division ? division : "Location"}</p>
                </button>
            </DialogTrigger>
            <DialogContent>
                <DialogHeader>
...
const DivisionSlide = ({ updateSearchParam }: { updateSearchParam: any }) => {
    ...
    return (
        <div className='max-h-[80vh] overflow-y-auto overflow-x-hidden'>
            {
                <>
                    <h4 className='text-lg font-epilogue font-medium text-black mb-3'>Select Division</h4>
                    <ul>
                        ...
                        <li className='w-full'>
                            <DialogTrigger asChild>
                                <button className='flex flex-row gap-x-5 justify-between items-center border-b border-stroke p-3 cursor-pointer hover:bg-gray-50 duration-200 w-full' onClick={() => {
                                    ...
                                    <p className='text-base font-epilogue'>All</p>
                                </button>
                            </DialogTrigger>
                        </li>

                        {divisions?.map(division => {
                            return <li key={`${division?.id}`} className='w-full'>
                                <DialogTrigger asChild>
                                    <button className='flex flex-row gap-x-5 justify-between items-center border-b border-stroke p-3 cursor-pointer hover:bg-gray-50 duration-200 w-full' onClick={() => {
                                        updateSearchParam({ division: division?.name })
                                    }}>
                                        <p className='text-base font-epilogue'>{division?.name}</p>
                                    </button>
                                </DialogTrigger>
                            </li>
                        })}
